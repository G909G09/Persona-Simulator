...
- 커밋 전에는 다음 검사를 실행하세요. 동일한 검사들이 GitHub Actions(`.github/workflows/check.yml`)에서 push·PR마다 자동으로 실행됩니다.
  - `node scripts/check-html-script-syntax.js persona-watch-v39.html persona-watch-observer.html` — 인라인 `<script>` 블록의 문법 오류를 확인합니다.
  - `node scripts/check-variant-sync.js` — 두 파일을 줄 단위로 비교해, 제목·저장 키·기본 설정값처럼 의도적으로 다른 부분(스크립트 안의 `ALLOWED_DIFFS` 목록)을 제외하고 예상치 못하게 어긋난 줄이 있으면 알려줍니다. 한쪽 파일만 고치고 다른 쪽에 반영하는 것을 잊었을 때 바로 잡아냅니다.
  - `node scripts/test-scripts.js` — 위 두 검사 스크립트 자체에 대한 회귀 테스트입니다.
  - 세 가지를 한 번에 실행하려면 `npm run check`(Node.js만 있으면 되고, 별도 의존성 설치는 필요 없습니다)를 사용하세요.
...
- Before committing, run the checks below. The same checks run automatically on every push/PR via GitHub Actions (`.github/workflows/check.yml`).
  - `node scripts/check-html-script-syntax.js persona-watch-v39.html persona-watch-observer.html` — catches syntax errors in the inline `<script>` blocks.
  - `node scripts/check-variant-sync.js` — line-by-line compares the two files and flags any unexpected divergence, aside from the documented intentional differences (title, save keys, default config values, etc. — see `ALLOWED_DIFFS` in the script). Catches the case where a fix was applied to only one of the two files.
  - `node scripts/test-scripts.js` — regression tests for the two check scripts themselves.
  - Or run all three at once with `npm run check` (Node.js only, no extra dependencies to install).
